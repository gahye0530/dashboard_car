...
def run_ml_app() :
    ...
    age = st.number_input('나이입력', df['Age'].min(), df['Age'].max())
    salary = st.number_input('연봉입력', df['Annual Salary'].min(), df['Annual Salary'].max())
    debt = st.number_input('카드빚입력', df['Credit Card Debt'].min(), df['Credit Card Debt'].max())
    worth = st.number_input('자산입력', df['Net Worth'].min(), df['Net Worth'].max())
    ...
    # 결과가 소수점으로 나오는데 소수점 뒤 한 자리까지만 나오도록 수정
    if btn : 
